...
-type state() :: #{
      ...
      , on_limit := [fun( (ticker(), [ticker()]) -> any() )]
    }.
    ...
    case klsn_map:lookup([tickers, Ticker], State0) of
        ...
        none ->
            case maps:get(available_rows, State0) of
                [] ->
                    %% No free rows – notify limit callbacks asynchronously.
                    Callbacks = maps:get(on_limit, State0, []),
                    CurrentTickers = maps:keys(maps:get(tickers, State0)),
                    lists:foreach(fun(F) ->
                        spawn(fun() -> F(Ticker, CurrentTickers) end)
                    end, Callbacks),
                    {noreply, State0};
                [Row | Rows] ->
...
%%--------------------------------------------------------------------
%% Callback management for limit‑reached events
%%--------------------------------------------------------------------

handle_cast({add_on_limit_callback, Fun}, State0) when is_function(Fun, 2) ->
    Callbacks0 = maps:get(on_limit, State0, []),
    State = State0#{on_limit => [Fun | Callbacks0]},
    {noreply, State};

handle_cast({remove_on_limit_callback, Fun}, State0) when is_function(Fun, 2) ->
    Callbacks0 = maps:get(on_limit, State0, []),
...
%%--------------------------------------------------------------------
%% Limit‑reached callback registration helpers
%%--------------------------------------------------------------------

-spec add_on_limit_callback(fun((ticker(), [ticker()]) -> any())) -> ok.
add_on_limit_callback(Fun) when is_function(Fun, 2) ->
    gen_server:cast(?MODULE, {add_on_limit_callback, Fun}).

-spec remove_on_limit_callback(fun((ticker(), [ticker()]) -> any())) -> ok.
remove_on_limit_callback(Fun) when is_function(Fun, 2) ->
